...
export default function AutoDetectScreen() {
  ...
  const parseSMSAndSave = async (smsBody: string): Promise<AutoDetectedExpense | null> => {
    try {
      ...
      if (parsed.is_duplicate) {
        ...
        Alert.alert('Duplicate', 'This transaction already exists');
        return null;
      }
  ...
  const simulateIncomingSMS = async () => {
    ...
    if (expense) {
      ...
      Alert.alert('Transaction Detected', `${expense.transaction_type === 'credit' ? '+' : '-'}\u20b9${expense.amount} - ${expense.description}`);
    }
  };
  ...
  const clearHistory = async () => {
    Alert.alert(
      'Clear History',
      'This will only clear the detection history from this screen.',
      [
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={[styles.statusCard, isListening && styles.statusCardActive]}>
          ...
          <Text style={styles.statusTitle}>
            {processing ? 'Processing SMS...' : isListening ? 'Listening for SMS' : 'Auto-detect is off'}
          </Text>
          ...
          <View style={styles.settingItem}>
            <View style={styles.settingInfo}>
              ...
              <View style={styles.settingText}>
                ...
                <Text style={styles.settingDescription}>Automatically detect expenses</Text>
              </View>
            </View>
            <Switch
          ...
          <View style={styles.settingItem}>
            <View style={styles.settingInfo}>
              ...
              <View style={styles.settingText}>
                ...
                <Text style={styles.settingDescription}>Test with sample SMS</Text>
              </View>
            </View>
            <Switch
        ...
        <View style={styles.infoBanner}>
          ...
          <Text style={styles.infoText}>
            Real SMS reading requires building as APK. Use simulation mode for testing.
          </Text>
        </View>
                ...
                >
                  ...
                  <Text
                    style={[
                      ...
                      { color: detection.transaction_type === 'credit' ? '#2ECC71' : '#FF6B6B' },
                    ]}
                  >
...
const styles = StyleSheet.create({
  ...
  scrollView: { flex: 1 },
  statusCard: { marginHorizontal: 16, marginTop: 8, backgroundColor: '#2d2d44', borderRadius: 16, padding: 24, alignItems: 'center', borderWidth: 2, borderColor: 'transparent' },
  statusCardActive: { borderColor: '#4ECDC4', backgroundColor: 'rgba(78, 205, 196, 0.1)' },
  statusIcon: { marginBottom: 12 },
  statusTitle: { color: '#fff', fontSize: 18, fontWeight: '600', marginBottom: 4 },
  statusSubtitle: { color: '#888', fontSize: 13, textAlign: 'center' },
  settingsSection: { marginHorizontal: 16, marginTop: 24 },
  sectionTitle: { color: '#fff', fontSize: 16, fontWeight: '600', marginBottom: 12 },
  settingItem: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#2d2d44', borderRadius: 12, padding: 16, marginBottom: 8 },
  settingInfo: { flexDirection: 'row', alignItems: 'center', flex: 1, gap: 12 },
  settingText: { flex: 1 },
  settingLabel: { color: '#fff', fontSize: 14, fontWeight: '500' },
  settingDescription: { color: '#888', fontSize: 12, marginTop: 2 },
  triggerButton: { flexDirection: 'row', backgroundColor: '#4ECDC4', marginHorizontal: 16, marginTop: 16, paddingVertical: 14, borderRadius: 12, alignItems: 'center', justifyContent: 'center', gap: 8 },
  ...
  detectionsSection: { marginHorizontal: 16, marginTop: 24 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  clearText: { color: '#FF6B6B', fontSize: 14 },
  emptyState: { alignItems: 'center', paddingVertical: 32, backgroundColor: '#2d2d44', borderRadius: 12 },
  emptyText: { color: '#888', fontSize: 14, marginTop: 12 },
  detectionItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#2d2d44', borderRadius: 12, padding: 12, marginBottom: 8 },
  detectionIcon: { width: 40, height: 40, borderRadius: 10, justifyContent: 'center', alignItems: 'center' },
  detectionDetails: { flex: 1, marginLeft: 12 },
  detectionDescription: { color: '#fff', fontSize: 13, fontWeight: '500' },
  detectionMeta: { color: '#888', fontSize: 11, marginTop: 4 },
  detectionAmount: { fontSize: 13, fontWeight: '600' },
